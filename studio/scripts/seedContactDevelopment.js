...
const PAGE_DOCUMENT_ID = 'contactPage-main'
const PAGE_TYPE = 'contactPage'
const PROJECT_ROOT = resolve(__dirname, '../..')
const FALLBACK_PATH = resolve(PROJECT_ROOT, 'data/contact-fallback.json')
...
function assertDevelopmentDataset(client) {
  ...
  if (dataset !== EXPECTED_DATASET) {
    throw new Error(
      `Contact-seed afgebroken: alleen dataset "${EXPECTED_DATASET}" is toegestaan.`,
    )
  }
...
function resolveImagePath(relativeUrl) {
  if (typeof relativeUrl !== 'string' || relativeUrl.trim() === '') {
    throw new Error('Contact-seed vereist een lokaal afbeeldingspad.')
  }
...
function buildSeedPlan({readFile = readFileSync} = {}) {
  ...
  try {
    ...
    throw new Error('Contact-fallback kon niet veilig worden gelezen.')
  }

  if (
    !Array.isArray(content.contactTopics) ||
    content.contactTopics.length !== 4
  ) {
    throw new Error('Contact-seed vereist exact vier contactonderwerpen.')
  }

  const imageSources = [
    imageSource('hero', content.heroImageUrl, readFile),
    imageSource('welcome', content.welcomeImageUrl, readFile),
  ]
...
function createDocument(content, assets) {
  ...
  return {
    ...
    welcomeImage: imageReference(assets.get('welcome')),
    contactTopics: contactTopics.map((item, index) => ({
      _type: 'contactTopic',
      _key: `contact-topic-${index + 1}`,
      ...item,
    })),
  }
}
...
async function runSeed(
  ...
  if (wrongType) {
    throw new Error(
      `Contact-seed afgebroken: ${wrongType._id} heeft type ${wrongType._type}.`,
    )
  }

  if (existing.some(({_id}) => _id === `drafts.${PAGE_DOCUMENT_ID}`)) {
    throw new Error('Contact-seed afgebroken: singletondraft bestaat al.')
  }
